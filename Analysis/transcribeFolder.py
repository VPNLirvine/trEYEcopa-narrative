...
pattern = re.compile(r'Q(\d+)')  # pre-compile regex searching for e.g. "-Q1" or "-f_Q99"

def process_file(audio_file: str) -> str:
    """Load audio, transcribe with Whisper, return transcription text."""
    audio = whisper.load_audio(audio_file)
    audio = whisper.pad_or_trim(audio) # forces each file to be exactly 30 sec long
...
def main():
    Tk().withdraw() # prevents an unnecessary window from opening
    ...
    for i, filepath in enumerate(input_folder.iterdir(), 1):
        ...
        try:
            parts = base.split("-")
            subject_identifier = parts[0] # "TC_50"
            trial_num = int(parts[1])
            match = pattern.search(base) # e.g. "-f_Q65"
            if not match:
                raise ValueError("No animation ID found in filename")
            anim_id = int(match.group(1)) # extract the numbers after Q
        except ValueError as e:
            print(f"Filename parse failed for {filepath.name}: {e}")
            continue

        print(f"Processing file {i}: {filepath.name}")
        transcription = process_file(str(filepath)) # generate transcription
        row = [trial_num, anim_id, transcription]
        subject_data.setdefault(subject_identifier, []).append(row) # add this trial's data to a list
